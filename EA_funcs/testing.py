...
def evaluate_out_of_sample(weights, test_data, expected_daily_returns=None, plot=True, verbose=True):
    """
    ...
    # Expected return based on training returns
    if expected_daily_returns is not None:
        expected_return = (expected_daily_returns @ weights) * len(test_data)
    else:
        print("!")
        expected_return = portfolio_returns.mean() * len(test_data)
...
def test_portfolio_weights(weights, train_data, test_data, plot=True, verbose=True):
    ...
    """
    daily_returns_train = train_data.pct_change().dropna()
    expected_daily_returns = daily_returns_train.mean()
    cov_matrix = daily_returns_train.cov().values

    train_return = expected_daily_returns @ weights * len(test_data)
    train_risk = get_risk(weights, cov_matrix)

    if verbose:
        print(f"Expected return (train): {round(100 * train_return, 3)}%")
        print(f"Risk (train): {round(100 * train_risk, 3)}%")

    return evaluate_out_of_sample(weights, test_data, expected_daily_returns, plot, verbose)